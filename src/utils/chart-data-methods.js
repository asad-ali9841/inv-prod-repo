...
module.exports.getTotalInventoryValueChartData = async ({
  chartType,
  comparedTo,
  warehouseId,
  startDateStr,
  ...
  if ([ChartType.BarChart, ChartType.LineChart].includes(chartType)) {
    ...
    if (isSameDay) {
      ...
      const currentValue = snapshot.length
        ...
        : 0;
      const currentRoundedValue = Number(currentValue.toFixed(2));
    ...
    const currentRoundedValue = Number(currentValue.toFixed(2));
    const previousRoundedValue = Number(previousValue.toFixed(2));
    ...
    return {
      ...
      differential: Number(percentageChange.toFixed(2)),
      differenceStatus,
    };
  }
};
