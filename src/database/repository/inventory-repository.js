  ...
  async duplicateProductV3(productKey, userInfo) {
    ...
    try {
      ...
      for (let i = 0; i < originalProduct.variantIds.length; i++) {
        ...
        const barcodeType = originalVariant.barcodeType;
        const autoGeneratedBarcode = originalVariant.autoGeneratedBarcode;
        const barcodeValue = originalVariant.barcodeValue;

        // Generate barcode
        const barcodeSvg = createBarcode(
          barcodeType,
          autoGeneratedBarcode ? newVariantId : barcodeValue
        );
        ...
        newDupVar.autoGeneratedBarcode = autoGeneratedBarcode;
        newDupVar.barcodeValue = barcodeValue;
        newDupVar.barcodeType = barcodeType;
...
async function createNewItems(
  ...
  for (const itemPayload of newItemPayloads) {
    ...
    const barcodeType = itemPayload.barcodeType;
    const autoGeneratedBarcode = itemPayload.autoGeneratedBarcode;
    const barcodeValue = itemPayload.barcodeValue;

    // generate barcode
    const barcodeSvg = createBarcode(
      barcodeType,
      autoGeneratedBarcode ? variantId : barcodeValue
    );
