...
class InventoryService {
  ...
  */
      ...
      for (const payload of bulkPayloads) {
        ...
        if (Array.isArray(payload.variants)) {
          payload.variants.forEach((variantData, i) => {
            ...
            // TODO: remove this when the import is updated to support barcodes
            const barcodeType = variantData.barcodeType || 'code128';
            // TODO: remove this when the import is updated to support barcodes
            const autoGeneratedBarcode = variantData.autoGeneratedBarcode || true;
  ...
  async getChartData(queryParams, authKey) {
    try {
      const chartData = await this.respository.getChartData(queryParams, authKey);
      return apiPayloadFormat(
        1,
        "success",
